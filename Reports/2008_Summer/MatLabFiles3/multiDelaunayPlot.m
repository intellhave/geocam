...
%stop for t seconds between each iteration. If no t value is given, for
%options 1 and 2 the user will manually have to scroll through the
%iterations by pressing a key while clicked on the figure. Option 3 does is
%not affected by the third option, so it can be entered or not.
%
...
if option == 1
    for i = 0:k
        ...
        %j = ['C:\Dev-Cpp\geocam\Triangulations\notable examples\Animated 2 (non-negative flip loop)\Step ' h '.txt'];
