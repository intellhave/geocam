...
double getSlope(Edge e)
{
       ...
       if((v1.getXpos()) != (v2.getXpos()))
           return ((v1.getYpos() - v2.getYpos()) / (v1.getXpos() - v2.getXpos()));


}
vector<char> getDirection(Edge e, Vertex v)
{
       vector<char> dir;
       Vertex d;
       if((*(e.getLocalVertices()))[0] == v.getIndex())
             d = Triangulation::vertexTable[(*(e.getLocalVertices()))[1]];
       else
             d = Triangulation::vertexTable[(*(e.getLocalVertices()))[0]];
       
       if(d.getYpos() > v.getYpos())
             dir[0] = 'U';
       else
             dir[0] = 'D';
       
       if(d.getXpos() > v.getXpos())
             dir[1] = 'R';
       else
             dir[1] = 'L';
