...
// GeoQuants used (explicitly) in this calculation.
#include "radius.cpp"
#include "eta.cpp"
...
void Newtons_Method() {
  ...
  while(true) {            
    ...
    for(vit = Triangulation::vertexTable.begin(); vit != Triangulation::vertexTable.end(); vit++) {
      X_n[(vit->first)-1] = Solution[(vit->first)-1] + X_n[(vit->first)-1];
...
double EHR_Second_Partial (int i, int j) {
  ...
  result = pow(V, (-4.0/3.0))*(1.0/3.0)*( 
     3*V*Curvature_Partial(i,j)
