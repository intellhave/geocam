...
public class StdFace {
  ...
  // orientation stuff
  public static StdFace getOrientedFace(Face embeddedFace) {
    if (orientation.containsKey(embeddedFace)) {
      return orientation.get(embeddedFace);
    } else {
      return null;
    }
  }
  ...
  private static void generateOrientationHelper(Face embeddedFace, Vertex firstVert, Edge firstEdge) {
    if (orientation.containsKey(embeddedFace)) {
      return;
    }

    StdFace stdFace = new StdFace(embeddedFace, firstVert, firstEdge);
    orientation.put(embeddedFace, stdFace);

    for (Face f : embeddedFace.getLocalFaces()) {
