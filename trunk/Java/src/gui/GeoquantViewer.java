...
public class GeoquantViewer extends javax.swing.JFrame implements ItemListener {
	...
	private JCheckBox lkCurvatureCheck;
	...
	private JMenuItem conformalDiskMenuItem;
	private ConformalDiskFlowDialog conformalDiskFlowDialog;
	private AbstractAction showConformalDiskFlowDialog;
		...
		try {
			...
			this.setResizable(false);
			{
				...
				setJMenuBar(mainMenuBar);
				{
						...
						public void actionPerformed(ActionEvent arg0) {
							if (!importedTriangulation)
								JOptionPane.showMessageDialog(null,
										"Must import triangulation first");
							else{
								if (dev != null) {
									dev.vc.endSession();
									dev.dui.terminate();
					...
					showExponentialView
							.addActionListener(ShowExponentialViewListener);
		...
		if (importAction == null) {
			importAction = new AbstractAction("Import Triangulation", null) {
				public void actionPerformed(ActionEvent e) {
					...
					getTriangulationFileChooser().setFileFilter(
							TXTFilter.getFilter());
					getTriangulationFileChooser().setFileFilter(
							XMLFilter.getFilter());
					int returnVal = getTriangulationFileChooser()
							.showOpenDialog(GeoquantViewer.this);

					if (returnVal == JFileChooser.APPROVE_OPTION) {
						File file = getTriangulationFileChooser()
								.getSelectedFile();
						TriangulationIO.readTriangulation(file
								.getAbsolutePath());
						newTriangulation();

						System.out.println("Vertex Count: "+Triangulation.vertexTable.size());
						System.out.println("Face Count: "+Triangulation.faceTable.values().size());
						System.out.println("Edge Count: "+Triangulation.edgeTable.values().size());
			...
			importAction.putValue(javax.swing.Action.ACCELERATOR_KEY,
			...
			saveAction = new AbstractAction("Save Triangulation", null) {
				public void actionPerformed(ActionEvent e) {
					...
					getTriangulationFileChooser().setFileFilter(
							XMLFilter.getFilter());
					int returnVal = getTriangulationFileChooser()
							.showSaveDialog(GeoquantViewer.this);

					if (returnVal == JFileChooser.APPROVE_OPTION) {
						File file = getTriangulationFileChooser()
								.getSelectedFile();
						TriangulationIO.writeTriangulation(file
								.getAbsolutePath());
					}
				}
			};
			saveAction.putValue(javax.swing.Action.ACCELERATOR_KEY,
	...
	private void newTriangulation() {
		// Modify Edge List
		EdgeListModel = new DefaultComboBoxModel(Triangulation.edgeTable
				.values().toArray());
		...
		if (Triangulation.getDimension() == 2) {
			...
			conformalDiskMenuItem.setEnabled(true);
			...
			getLKCurvatureCheck().setEnabled(true);
			...
			conformalDiskMenuItem.setEnabled(false);
			...
			getLKCurvatureCheck().setEnabled(false);
	...
	private JPanel getQuantityModPanel() {
		if (quantityModPanel == null) {
			...
			GroupLayout quantityModPanelLayout = new GroupLayout(
					(JComponent) quantityModPanel);
			quantityModPanel.setLayout(quantityModPanelLayout);
			quantityModPanel.setBorder(BorderFactory
			...
			quantityModPanelLayout
					.setHorizontalGroup(quantityModPanelLayout
							.createSequentialGroup()
							.addContainerGap()
							.addGroup(
									quantityModPanelLayout
											...
											.addComponent(
													getEdgeDisplayPanel(),
													GroupLayout.Alignment.LEADING,
													0, 421, Short.MAX_VALUE))
							.addContainerGap());
			quantityModPanelLayout
					.setVerticalGroup(quantityModPanelLayout
							.createSequentialGroup()
							.addContainerGap()
							.addGroup(
									quantityModPanelLayout
							...
							.addComponent(getEdgeDisplayPanel(),
	...
	private JScrollPane getEdgeListScrollPane() {
		if (edgeListScrollPane == null) {
			...
			{
				EdgeListModel = new DefaultComboBoxModel();
				EdgeList = new JList();
	...
	private JRadioButton getPerpBisectorRadioButton() {
		if (perpBisectorRadioButton == null) {
			...
			perpBisectorRadioButton
					.addActionListener(new AlphaButtonListener());
		...
		if (edgeDisplayPanel == null) {
			edgeDisplayPanel = new JPanel() {
				protected void paintComponent(Graphics g) {
					...
					g.drawLine(size.width / 3, size.height / 2,
					...
					g.drawOval(size.width / 3 - arcLength, size.height / 2
							- arcLength, arcLength * 2, arcLength * 2);
					arcLength = (int) (rad2 * lineLength / length);
					g.drawOval(2 * size.width / 3 - arcLength, size.height / 2
							- arcLength, arcLength * 2, arcLength * 2);
				}
			};
			edgeDisplayPanel.setBackground(new java.awt.Color(255, 255, 255));
			edgeDisplayPanel.setBorder(new LineBorder(new java.awt.Color(0, 0,
					0), 1, false));
			edgeDisplayPanel
					.addMouseMotionListener(new EdgePanelMouseListener());
		}
	...
	private JPanel getBasicSelectPanel() {
		if (basicSelectPanel == null) {
			...
			basicSelectPanel.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.LOWERED));
			...
			basicSelectPanel.add(getLKCurvatureCheck());
		}
	...
	protected GeoPolygonPanel getPolygonPanel() {
		if (currentGeoPanel == null) {
			...
			GroupLayout currentGeoPanelLayout1 = new GroupLayout(
					(JComponent) currentGeoPanel);
			currentGeoPanel.setLayout(currentGeoPanelLayout1);
			GroupLayout currentGeoPanelLayout = new GroupLayout(
					(JComponent) currentGeoPanel);
			currentGeoPanel.setOwner(this);
			currentGeoPanel.setBackground(new Color(255, 255, 255));
			currentGeoPanel.setBorder(new LineBorder(
					new java.awt.Color(0, 0, 0), 1, false));
			currentGeoPanel.setSize(378, 378);
			currentGeoPanelLayout.setVerticalGroup(currentGeoPanelLayout
					.createSequentialGroup());
			currentGeoPanelLayout.setHorizontalGroup(currentGeoPanelLayout
					.createSequentialGroup());
			currentGeoPanelLayout1
					.setHorizontalGroup(currentGeoPanelLayout1
							.createSequentialGroup()
							.addContainerGap()
							.addGroup(
									currentGeoPanelLayout1
											...
											.addGroup(
													...
													currentGeoPanelLayout1
															...
															.addGap(0,
																	...
																	Short.MAX_VALUE)))
							.addContainerGap());
			currentGeoPanelLayout1.setVerticalGroup(currentGeoPanelLayout1
					.createSequentialGroup()
					.addContainerGap()
					.addComponent(getShowFlowButton(),
					...
					.addGroup(
							currentGeoPanelLayout1
									.createParallelGroup(
											GroupLayout.Alignment.BASELINE)
									.addComponent(getAnimateButton(),
											GroupLayout.Alignment.BASELINE,
											GroupLayout.PREFERRED_SIZE,
											GroupLayout.PREFERRED_SIZE,
											GroupLayout.PREFERRED_SIZE)
									.addComponent(getStepLabel(),
											GroupLayout.Alignment.BASELINE,
					...
					.addGroup(
							currentGeoPanelLayout1
									.createParallelGroup(
											GroupLayout.Alignment.BASELINE)
									.addComponent(getStepComboBox(),
											GroupLayout.Alignment.BASELINE,
											GroupLayout.PREFERRED_SIZE,
											GroupLayout.PREFERRED_SIZE,
											GroupLayout.PREFERRED_SIZE)
									.addComponent(getDelayLabel(),
											GroupLayout.Alignment.BASELINE,
											GroupLayout.PREFERRED_SIZE, 26,
											GroupLayout.PREFERRED_SIZE)
									.addComponent(getDelayField(),
											GroupLayout.Alignment.BASELINE,
											GroupLayout.PREFERRED_SIZE, 26,
											GroupLayout.PREFERRED_SIZE))
					.addContainerGap());
			currentGeoPanelLayout1.linkSize(SwingConstants.VERTICAL,
					new Component[] { getStepLabel(), getDelayLabel() });
		}
		...
		public void valueChanged(ListSelectionEvent e) {
			if (!e.getValueIsAdjusting()) {
				...
				if (EdgeList.getSelectedIndex() == -1) {
					...
					Edge s = (Edge) EdgeListModel.getElementAt(EdgeList
							.getSelectedIndex());
					Vertex v1 = s.getLocalVertices().get(0);
					Vertex v2 = s.getLocalVertices().get(1);
	...
	class EdgePanelMouseListener extends MouseAdapter {
		...
		public EdgePanelMouseListener() {
			...
			message = new JLabel();
			message.setBorder(new LineBorder(new java.awt.Color(0, 0, 0), 1,
					false));
		}
		...
		public void mouseMoved(MouseEvent e) {
			...
			if (x > (width / 3 - epsilon) && x < (2 * width / 3 + epsilon)
				...
				edgeDisplayPopup = factory.getPopup(GeoquantViewer.this,
						message, (int) edgeDisplay.getLocationOnScreen().getX()
								+ x, (int) edgeDisplay.getLocationOnScreen()
								.getY() + y - 15);
	...
	// public GeoPolygon(int[] xpoints, int[] ypoints, int npoints, Object[]
	// geos) {
	...
	private JTabbedPane getJTabbedPane1() {
		if (geoSelectTabbedPane == null) {
			...
			geoSelectTabbedPane.addTab("Basic Quantities", null,
					getBasicSelectPanel(), null);
			geoSelectTabbedPane.addTab("Partials", null,
					getPartialSelectPanel(), null);
			geoSelectTabbedPane.addTab("Second Partials", null,
					getSecondPartialSelectPanel(), null);
		}
	...
	private JPanel getPartialSelectPanel() {
		if (partialSelectPanel == null) {
			...
			partialSelectPanel
					.setPreferredSize(new java.awt.Dimension(485, 158));
			partialSelectPanel.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.LOWERED));
			partialSelectPanel.setEnabled(false);
	...
	private JPanel getSecondPartialSelectPanel() {
		if (secondPartialSelectPanel == null) {
			...
			GroupLayout seconPartialSelectPanelLayout = new GroupLayout(
					(JComponent) secondPartialSelectPanel);
			secondPartialSelectPanel.setLayout(seconPartialSelectPanelLayout);
			secondPartialSelectPanel.setBorder(BorderFactory
					.createEtchedBorder(BevelBorder.LOWERED));
			secondPartialSelectPanel.setEnabled(false);
			seconPartialSelectPanelLayout
					.setHorizontalGroup(seconPartialSelectPanelLayout
							.createSequentialGroup()
							.addContainerGap()
							.addGroup(
									seconPartialSelectPanelLayout
											...
											.addGroup(
													seconPartialSelectPanelLayout
															.createSequentialGroup()
															.addComponent(
																	getTotalVolumeSecondPartialCheck(),
																	GroupLayout.PREFERRED_SIZE,
																	191,
																	GroupLayout.PREFERRED_SIZE)
															.addGap(0,
											...
											.addGroup(
													seconPartialSelectPanelLayout
															.createSequentialGroup()
															.addComponent(
																	getCurvatureSecondPartialCheck(),
																	GroupLayout.PREFERRED_SIZE,
																	193,
																	GroupLayout.PREFERRED_SIZE)
															.addGap(0,
											...
											.addGroup(
													...
													seconPartialSelectPanelLayout
															.createSequentialGroup()
															.addComponent(
																	getPartialEdgeSecondPartialCheck(),
																	GroupLayout.PREFERRED_SIZE,
																	159,
																	GroupLayout.PREFERRED_SIZE)
															.addGap(0,
			...
			seconPartialSelectPanelLayout
					.setVerticalGroup(seconPartialSelectPanelLayout
							.createSequentialGroup()
							.addContainerGap()
							.addGroup(
									seconPartialSelectPanelLayout
			...
			seconPartialSelectPanelLayout.linkSize(SwingConstants.VERTICAL,
					new Component[] { getCurvatureSecondPartialCheck(),
							getDihAngleSecondPartialCheck(),
							getVehrSecondPartialCheck(),
							getPartialEdgeSecondPartialCheck(),
							getVolumeSecondPartialCheck(),
	...
	private JCheckBox getLKCurvatureCheck() {
		if (lkCurvatureCheck == null) {
			lkCurvatureCheck = new JCheckBox();
			lkCurvatureCheck.setText("LK Curvature");
			lkCurvatureCheck.setBounds(324, 148, 148, 16);
			lkCurvatureCheck.setEnabled(false);
			getGeoCheckTable().put(lkCurvatureCheck, LKCurvature.class);
			lkCurvatureCheck.addItemListener(this);
		}
		return lkCurvatureCheck;
	}
	...
	private JCheckBox getDihAnglePartialCheck() {
		if (dihAnglePartialCheck == null) {
			...
			getGeoCheckTable().put(dihAnglePartialCheck,
					DihedralAngle.Partial.class);
	...
	private JCheckBox getPartialEdgePartialCheck() {
		if (partialEdgePartialCheck == null) {
			...
			getGeoCheckTable().put(partialEdgePartialCheck,
					PartialEdge.Partial.class);
	...
	private JCheckBox getCurvatureSecondPartialCheck() {
		if (curvatureSecondPartialCheck == null) {
			...
			getGeoCheckTable().put(curvatureSecondPartialCheck,
					Curvature3D.SecondPartial.class);
	...
	private JCheckBox getDihAngleSecondPartialCheck() {
		if (dihAngleSecondPartialCheck == null) {
			...
			getGeoCheckTable().put(dihAngleSecondPartialCheck,
					DihedralAngle.SecondPartial.class);
	...
	private JCheckBox getVehrSecondPartialCheck() {
		if (vehrSecondPartialCheck == null) {
			...
			getGeoCheckTable().put(vehrSecondPartialCheck,
					VEHR.SecondPartial.class);
	...
	private JCheckBox getPartialEdgeSecondPartialCheck() {
		if (partialEdgeSecondPartialCheck == null) {
			...
			getGeoCheckTable().put(partialEdgeSecondPartialCheck,
					PartialEdge.SecondPartial.class);
	...
	private JCheckBox getVolumeSecondPartialCheck() {
		if (volumeSecondPartialCheck == null) {
			...
			getGeoCheckTable().put(volumeSecondPartialCheck,
					Volume.SecondPartial.class);
	...
	private JMenu getRunMenu() {
		if (runMenu == null) {
			...
			runMenu.add(getConformalDiskMenuItem());
		}
		return runMenu;
	}
	...
	private YamabeFlowDialog getYamabe3DDialog() {
		if (yamabe3DDialog == null) {
			yamabe3DDialog = new YamabeFlowDialog(this,
					Geometry.Dimension.threeD);
		}
	...
	private JMenuItem getConformalDiskMenuItem() {
		if (conformalDiskMenuItem == null) {
			conformalDiskMenuItem = new JMenuItem();
			conformalDiskMenuItem.setText("Conformal Disk/LK Curvature Flow");
			conformalDiskMenuItem.setAction(getShowConformalDiskFlowDialog());
			conformalDiskMenuItem.setEnabled(false);
		}
		return conformalDiskMenuItem;
	}

	private AbstractAction getShowConformalDiskFlowDialog() {
		if (showConformalDiskFlowDialog == null) {
			showConformalDiskFlowDialog = new AbstractAction(
					"Conformal Disk/LK Curvature Flow", null) {
				public void actionPerformed(ActionEvent evt) {
					getConformalDiskFlow().pack();
					getConformalDiskFlow().setLocationRelativeTo(null);
					getConformalDiskFlow().setVisible(true);
				}
			};
		}
		return showConformalDiskFlowDialog;
	}

	private ConformalDiskFlowDialog getConformalDiskFlow() {
		if (conformalDiskFlowDialog == null) {
			conformalDiskFlowDialog = new ConformalDiskFlowDialog(this);
		}
		return conformalDiskFlowDialog;
	}
	...
	private JCheckBox getTotalVolumePartialCheck() {
		if (totalVolumePartialCheck == null) {
			...
			getGeoCheckTable().put(totalVolumePartialCheck,
					Volume.PartialSum.class);
	...
	private JCheckBox getTotalVolumeSecondPartialCheck() {
		if (totalVolumeSecondPartialCheck == null) {
			...
			getGeoCheckTable().put(totalVolumeSecondPartialCheck,
					Volume.SecondPartialSum.class);
	...
	private JComboBox getStepComboBox() {
		if (stepComboBox == null) {
			ComboBoxModel stepComboBoxModel = new DefaultComboBoxModel(
					new String[] { "Step" });
	...
	private void buildStepComboBox() {
		...
		ComboBoxModel stepComboBoxModel = new DefaultComboBoxModel(stepList);
	...
	public void setDevelopment(Development dev){
		this.dev = dev;
	}
	...
	public class Development extends Thread {
		...
		public void run() {
			while (true) {
				dui.run(); // We only return from this call once
							// ViewerController has
							// signaled that we should quit the simulation.
				if (vc.sessionEnded()) {
					dui.terminate();
					vc.dispose();
					GeoquantViewer.this.setDevelopment(null);
					break;
				}
				dui = new DevelopmentUI();
				// Now we have a fresh, initialized DevelopmentUI instance.
				// We're ready to
