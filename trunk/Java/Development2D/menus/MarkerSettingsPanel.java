...
public class MarkerSettingsPanel extends JPanel {
	...
	numMarkersTextbox = new JFormattedTextField(
	...
	speedBorder.setTitle("Object Speed ("
		+ speedFormat.format(MAX_SPEED / 4) + ")");
	...
	numMarkersTextbox.addActionListener(new ActionListener() {
		...
		Integer numMarkers = Integer.parseInt(numMarkersTextbox
			.getValue().toString());
		if (numMarkers < 0) {
		    numMarkers = 0;
	...
	sizeSlider.addChangeListener(new ChangeListener() {
		...
		currentSize = sizeSlider.getValue() / 10.0;
		sizeBorder.setTitle("Object Size (" + sizeSlider.getValue()
			/ 10.0 + ")");
	...
	if (currentMarkers < numMarkers) {
		...
		pos = new ManifoldPosition(markerHandler.getSourceMarker()
			.getPosition());
