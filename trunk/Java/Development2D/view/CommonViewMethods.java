...
public class CommonViewMethods {
  ...
  protected static SceneGraphComponent generateDevelopmentObjectGeometry(
      DevelopmentNode devRoot, boolean clipNear, double clipNearRadius) {
    ...
    for (VisiblePath p : pathList) {
      ...
      sgcNewObjects.addChild(SGCMethods.pathSGCFromList(images,
          p.getAppearance(), true, 0));
    }
  ...
  public static void getDevelopmentObjectImages(DevelopmentNode devNode,
    ...
    if (objectList != null) {
      ...
      synchronized (objectList) {
        for (VisibleMarker o : objectList) {
          ...
          if (frustum != null) {
            if (!frustum.checkInterior(transPos)) {
              continue;
            }
          }
          // check if object should be clipped by specified
          // clipNearRadius (ok, sqrt(radius), but who cares)
          if (transPos.lengthSquared() < clipNearRadius) {
            continue;
          }
    ...
    while (itr.hasNext()) {
      getDevelopmentObjectImages(itr.next(), objectImages, clipNear,
          clipNearRadius);
    }
  }
    ...
    if (objectList != null) {
      ...
      synchronized (objectList) {
        for (VisibleMarker o : objectList) {
          ...
          Vector transPos = affineTrans.affineTransPoint(o.getPosition());
          // check if object image should be clipped by frustum
          if (frustum != null && (!frustum.checkInterior(transPos))) continue;
  ...
  private static void getDevelopmentPathImages(DevelopmentNode devNode,
    ...
    if (pathList != null) {
      ...
      for (VisiblePath p : pathList) {
        ...
        Collection<ManifoldPath.Segment> segments = p
            .getPathSegmentsInFace(devNode.getFace());
        if (segments == null) {
          continue;
        }
        ...
        for (ManifoldPath.Segment s : segments) {
          ...
          LineSegment cs = LineSegment.clipWithFrustum(frustum,
              new LineSegment(affineTrans.affineTransPoint(s.startPos),
