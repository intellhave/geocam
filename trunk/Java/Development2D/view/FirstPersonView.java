...
public class FirstPersonView extends ExponentialView {
	...
	public FirstPersonView(Development dev, MarkerHandler mh) {
		super(dev, mh); // This call initializes the sgcpools data structure.
	...
	protected void updateCamera() {
		...
		// Generally, the smaller we make the altitude of the camera, the harder
		// it
	...
	protected void generateMarkerGeometry() {
		...
		for (Marker m : allMarkers) {
			...
			pool = super.getBigEnoughPool(m, imagesNeeded);
			...
			for (SceneGraphComponent sgc : pool) {
				if (counter >= images.size()) {
					...
					// In the future, we should have a dedicated SGC pointer for
					// the
					// avatar.
					double epsilon = 0.05;
	 ...
	 * particular marker) is drawn right in front of the camera (to represent
	 * the user/player).
	 *********************************************************************************/
