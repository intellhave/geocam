...
\includegraphics[scale = 0.5]{badcase2.png}
...
\subsection{Definition}
We define Delaunay triangulations as a set of faces on a plane such that the circumcircle defined by the three vertices of any given face do not enclose any other vertices. See Fig.~\ref{genTri} for an example with only a few faces. There are two main Delaunay cases, weighted and non-weighted. In non-weighted, all vertices have face value, and the circumcircle about the face is made to intersect all three vertices. In a weighted Delaunay case, some vertices have a larger influence than others. The circumcircle surrounding a face uses the weight of each vertex. We use the term weight in a similar way as with our Ricci flows. In fact, our previous cases of circle packing \textit{are} weighted Delaunay triangulations. \newline
...
\section{Modifications to our data structure}
...
\begin{figure}
\begin{center}
\includegraphics[scale = 0.6]{intcirc2.png}
\end{center}
