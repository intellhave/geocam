...
Up til now we have focused solely on triangulations using planar Euclidean geometry. While this is useful for most cases, there are others where we may need to change the background space. Two of these in particular are spherical and hyperbolic geometries. We will introduce the basics of each system to the reader as they may be unfamilar with these geometries. We will examine the adaptations of combinatorial Ricci flow for each case, test over various triangulations, and attempt to reach conclusions on our findings. 

\subsection{Spherical flow}
If you were to draw a large triangle on the surface of the earth, you would see that the line segments are no longer linear, but curve around along a circular path. In dealing with spherical geometry, we learn that many rules of planar geometry, some of them fundamental, do not apply. To begin, the sums of angles in a triangle do not add to $180^\circ$. The sum changes depending on the edge lengths. For computational reasons, we cannot (easily) have edges of unbounded length. While we can assume our circle packing metric still holds, we do have additional restrictions: 

\begin{itemize}
\item All triangles must (initially) be producable on a sphere of radius 1, the unit sphere. Triangulations can be thought of as a culmination of spherical triangles from many different spheres. 
\item The sum of the weights of any triangle cannot exceed $\pi.$ This is partially due to the possibility of multiple triangles on a sphere using the same points, and also to inherent domain bounds in formulae. By default, we imply the geodesic of a spherical triangle, using the shortest distance on the surface between any two vertices.
\end{itemize}
...
\noindent More importantly, our equation for combintorial Ricci flow is altered slightly. The base equation is now 
\begin{equation}
\label{SRiccif}
...
\begin{figure}
\centering
\includegraphics[scale = 0.6]{octosph.png}
\label{OctoSph}
...
\noindent Like in the Eulidean case, we see the possibility that all weights could continually decrease towards zero. However, scaling in the spherical case is not quite as easy as before: the angles are affected by the change in edge lengths. Plus, we also need make sure that the sum of weights remains below $\pi$.\newline
...
\noindent Since the surface area went to $4\pi$, then the average curvature went to zero. While we liked the end result of our triangulation trials, the system failed far too often to be reliable. 
...
\begin{figure}
\centering
\includegraphics[scale = 0.65]{kHatResult.png}
...
\noindent In the end, we decided to make an equation that took the good aspects of our first trial while broadening its range of stability as we had hoped with the second one. We came up with the equation
...
\noindent In examining its behavior with various triangulations, we found that this one works very well. We're not completely sure why, but it does. Spheres converge to zero curvature and weights first group together and then approach optimal weights. See Fig.~\ref{SphGood}. In dealing with vertex transitive triangulations, ones in which each vertex has the same degree, we noted that sometimes all weights would converge to a specific value that was dependant on the number of vertices, and sometimes all the weights would be close to this number, but off by a small amount. In either case, the resulting surface area at the end turned out to be $4\pi$. For a vertex transitive genus 0 surface, we found that the preferred weight that each vertex approached or reached was equal to 

$$r_i = \frac{1}{2}\arccos(\frac{\cos(Z)}{1 - \cos(Z)}) \mbox{ where } Z = \frac{\pi(4 + F)}{3F}$$

\noindent and $F$ is the number of faces of the given triangulation. For example, with a tetrahedron, $F = 4, Z = \frac{2\pi}{3},$ and $r_i = \frac{1}{2}\arccos(-\frac{1}{3}) \approx 0.9553.$ The only downsides we found with this equation were that it was still possible for the system to fail if some weights got too large, and the computation time required to reach a stable equilibrium was lengthier than expected. Looking at Fig.~\ref{SphGood}, we see that the $dt$ size given, it would take over 4 minutes for the system to reach equilibrium in real time.  

\begin{figure}
\includegraphics[scale = 0.8]{SphG0V10.png}
...
In hyperbolic geometry, we encounter a new background that has properties both similar and distinct from Euclidean and spherical systems. Please see Fig.~\ref{PD} for an illustration of the hyperbolic plane. This notion of a hyperbolic plane can also be seen in popular art, particularly M.C. Escher's ``Circle Limit IV,'' commonly referred to as the ``Angels and Devils'' picture. \newline

\noindent The major new difference with the hyperbolic plane is that our traditional notion of unique parallel lines no longer apply. We now have that for a point $P$ on a line $l$, there is a point $B$ on a line $m$ such that $B$ and $P$ create a line $n \perp l$, and there are an infinite number of possible $m$'s. The shortest distance between two points is still a straight line, but illustratively can be found by following a curved path along a circle centered at infinity going through both points. \newline

\noindent In terms of the formulas, they remain relatively similar in format to the spherical equations, except there are two main substitutions. The cosine and sine functions are often replaced with their hyperbolic counterparts $cosh$ and $sinh$ (pronounced ``synch''), defined as 
...
\noindent Like in the case of spherical flow, the average and total curvatures need not remain constant. The average curvature can be now found by
$$\overline{K} = \frac{\sum{K_i}}{|V|} = \frac{2\pi\chi + \mbox{Surface Area of }X}{|V|}.$$

\begin{figure}
\centering
\includegraphics[scale = 0.5]{pDisk.png}
\caption{An illustration of the hyperbolic plane using the Poincar\'{e} disk. named after the same Poincar\'{e} as in $\oint$\ref{RBk}. All concentric circles are evenly spaced apart in a Euclidean background, but the outer edge of the disk approaches infinity, so the circles appear closer together. A similar way to think of it is to imagine looking at the contour graph of $z = x^2 + y^2$ from the point (0,0,-1).}
\label{PD}
\end{figure}

\noindent After running the hyperbolic Ricci flow on a few samples, we noted that the weights did not always go to zero, as was the case with the previous unnormalized systems. So in some cases, it almost seems as if this equation is already normalized. Curvatures would converge to zero, and optimum, nonzero weights are achieved in a relatively short time. Other times it behaved exactly like both Euclidean and spherical. 

\subsection{Comparison of Systems}

When we began looking into these background geometries, we thought that it would be a good idea to run uniform tests on all three systems and observe how they behave. However, since our equation for normalized spherical Ricci flow is not firmly confirmed as the best method, and as the hyperbolic flow may or may not already be normalized, we can not make any strong conclusions on the normalized flows. We did decide, however, to take a look at the behaviors on various manifolds by using the non-normalized equations, (\ref{Riccif}), (\ref{SRiccif}), and (\ref{HRiccif}). We will look at three triangulations, each of a different genus, no morphs, and compare results between each system. \newline

\noindent Example: 12 vertex sphere, vertex transitive of degree 5\newline

Euclidean- As expected, we obtain a surface where all the vertices attain the same curvature of $\frac{2\pi}{5}$ after multiple trials of uniform and randomized weights. There are 20 faces in the resulting triangulation. The weights group together somewhat as in the normalized spherical case, and then continue decreasing towards zero. 

Spherical- We found that the system was stable only if all initial weights were set exactly to $\approx$ 0.55357, its preferred weight. If the initial curvature was below zero, the weights would increase until the program crashed. If the initial curvature was above zero, the vertices would approach the same curvature as in the Euclidean case. As such, the weights would drop to zero. 

Hyperbolic- We found that the vertices react in almost the exact same behavior as in the Euclidean case. They approach the same curvature of $\frac{2\pi}{5}$ and drop towards zero weights at the same rate. 

\noindent Example: 9 vertex, one-holed torus\newline

\noindent Example: 11 vertex, two-holed torus\newline

\noindent Systems work best for triangulations whose total curvature goes to zero in that particular system. Euclidean is best for systems of genus 1, or one-holed tori. This is the case as we saw the total curvature for such a torus was $2\pi\chi = 2\pi(2 - 2g) = 2\pi(2 - 2(1)) = 0$. Spherical combinatorial Ricci flow is said to work best on manifolds of genus 0, or anything topologically equivalent to a sphere. This is because we have a positive $\chi$ value, so for total curvature to go to 0, the total surface area must go to $4\pi$. Hyperbolic Ricci flow is best suited for triangulations with a genus of 2 or more. We can see that these systems will be able to reach zero total curvature with a large enough surface area. 

\section{Delaunay triangulations}
In addition to our work on combinatioral Ricci flow, we also branched out into other areas where we felt our triangulation structure would be applicable. We'll talk about this a lot more in detail as we get the code solidified. Tom 
\section{Future work}
\subsection{3-D}
...
\noindent Ultimately, Dr. Glickenstein would like to be able to build 3-D representations of these surfaces and walk along them in any given path. We can look into how faces are connected to each other, and be able to move from one triangle to another seamlessly. We can lay groundwork for that, and when this project is able to jump to 3-D modeling, we hope that this will help.  
...
\label{HypSphere}
Hence far, we have focused on Euclidean triangles. However, there are cases when triangulations may be better suited in other systems. 
