...
public class SimulationManager implements Runnable, Subscriber<MarkerSettings> {
	...
	private View firstPersonView = null;
	private View exponentialView = null;
	private View embeddedView = null;
	...
	public SimulationManager() {
		...
		if (development == null) {
			...
			development.rebuild(new ManifoldPosition(sourceFace, sourcePoint),
					3);
		}
	 ...
	 * removeFlaggedMarkers
	 * 
	 * This is a helper method for run. When a marker needs to be removed from
	 * the simulation, it indicates this by setting its markertype to REMOVED.
	 * In this method, we find all such markers and remove them from each part
	 * of the simulation.
	 *********************************************************************************/
	 ...
	 * initViews
	 * 
	 * This method initializes the data structures that keep track of the views
	 * in our program. These views will be initialized in ViewerController.
	 *********************************************************************************/
	private void initViews() {
		faceAppearanceScheme = new FaceAppearanceScheme();
		views = Collections
				.newSetFromMap(new ConcurrentHashMap<View, Boolean>());
	 ...
	 *********************************************************************************/
	private JFrame makeJFrame(View view, int locationX, int locationY) {
		JOGLViewer jv = view.getViewer();

		JFrame frame = new ViewFrame(this, view);
	...
	public void launchExponentialView() {
	...
	public void launchEmbeddedView() {
		if (!isEmbedded || embeddedView != null)
			return;
	...
	public void launchFirstPersonView() {
		...
		JFrame frame = makeJFrame(firstPersonView, 1210, 40);
	...
	public void closeFirstPersonView() {
		if (firstPersonView == null)
			return;
	...
	public void removeView(View view) {
		if (view == exponentialView) {
			closeExponentialView();
		} else if (view == embeddedView) {
			closeEmbeddedView();
		} else if (view == firstPersonView) {
			closeFirstPersonView();
		} else {
	...
	public void update(MarkerSettings data) {
		boolean initializing = (markerSettings == null);
		
		if(initializing){
			markerSettings = data;			
		}
	
		if(initializing || markerSettings.getNumMarkers() != data.getNumMarkers())
			setMovingMarkerCount( data.getNumMarkers() );
		
		if(initializing || markerSettings.getMarkerSpeed() != data.getMarkerSpeed())
			setMovingMarkerSpeed(data.getMarkerSpeed());
		
		if(initializing || markerSettings.getMarkerScale() != data.getMarkerScale())
			setMovingMarkerScale(data.getMarkerScale());
		
		if(initializing || markerSettings.moveMarkers() != data.moveMarkers())
			setMarkerMobility(data.moveMarkers());
		
		if(initializing || markerSettings.drawAvatar() != data.drawAvatar())
			setSourceVisible(data.drawAvatar());
		
		//if(initializing || markerSettings.drawMarkers() != data.drawMarkers()){
		// FIXME
		//}
		
		this.markerSettings = data;
	}
}
